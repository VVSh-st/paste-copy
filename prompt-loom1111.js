...
(function () {
  ...
  function record(text, source = 'manual', meta = {}) {
    ...
    const kind = classify(clean, source);
    if (shouldSkip(clean, source, kind)) return null;

    const hash = hashText(clean);
    const now = Date.now();
    const recentSame = state.items.find(x => x.hash === hash && now - (x.updatedAt || x.createdAt) < 15000);
    if (recentSame) {
      recentSame.meta = { ...(recentSame.meta || {}), lastVia: meta?.via || source };
      recentSame.updatedAt = Math.max(Number(recentSame.updatedAt || 0), now);
      saveState();
      renderPanelList();
      return recentSame;
    }

    const mergeTarget = findMergeTarget(clean, source, kind, hash, now);
    if (mergeTarget) {
      mergeSimilarItem(mergeTarget, clean, source, meta, now);
      saveState();
      renderPanelList();
  ...
  function getQuickItems(query = '', limit = QUICK_LIMIT) {
    ...
    if (cleanQuery) return getItems('all', cleanQuery, null);

    const scoredRecent = [...state.items]
      .sort((a, b) => quickRankScore(b) - quickRankScore(a) || b.createdAt - a.createdAt);

    const pinned = settings.quickPinned
      ? scoredRecent.filter(item => item.pinned).slice(0, 3)
      : [];
  ...
  function quickRankScore(item) {
    ...
    const baseTime = Math.max(Number(item.updatedAt || 0), Number(item.createdAt || 0));
    const ageHours = Math.max(0, (Date.now() - baseTime) / 3600000);
    const usedBoost = usedAt ? 1200000000000 + usedAt : 0;
    const useBoost = Math.min(uses, 9) * 1800000;
    const seenBoost = Math.min(Math.max(0, Number(item.seen || 1) - 1), 8) * 420000;
    const freshnessPenalty = Math.min(ageHours, 72) * 2500;
    return Math.max(baseTime, usedBoost) + useBoost + seenBoost - freshnessPenalty;
  ...
  function renderCard(item) {
    ...
    card.dataset.plTip = buildItemTitle(item);
    card.innerHTML = `
  ...
  function acceptPaletteIndex(idx) {
    ...
    record(item.text, 'snippet', { via: 'loom-quick' });
  ...
  function getInsertTarget(options = {}) {
    ...
    return document.querySelector('.block-textarea:focus') || document.querySelector('.block-textarea');
  }
  ...
  function getTextareaCaretPoint(el, pos) {
    ...
    try {
      ...
      mirror.style.width = Math.max(20, el.clientWidth - pl - pr) + 'px';
  ...
  function getSelectionStart(el) {
    ...
    return sel?.rangeCount ? getEditableValue(el).length : null;
  }
