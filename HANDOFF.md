...
3. ✅ **Кнопка в футере блока** — 🤖 перед кнопкой тезауруса
4. ✅ **Произвольный запрос** — "добавь эмодзи", "сделай короче", "перефразируй"
5. ✅ **Diff-панель** — отдельная панель под textarea с подсветкой изменений
6. ✅ **Принятие/отмена**: ✓ кнопка, ЛКМ вне=принять, ПКМ=отмена, Esc=отмена
