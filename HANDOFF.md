...
**Изменения:**
1. **Лимит** — максимум 20 пользовательских шаблонов, счётчик `N/20` в модалке
2. **Статистика** — `createdAt` + `useCount` в каждом шаблоне, дата и кол-во использований в snap-date, миграция старых
3. **Дублирование** — кнопка 📋, клонирует шаблон как «имя (копия)»
4. **Переименование** — dblclick по имени → inline- input, Enter/blur сохраняет, Escape отменяет
5. **Превью блоков** — hover на кнопку «+ Создать вкладку» → tooltip со списком блоков (icon + title + type), `position: absolute`, `pointer-events: none`, плавная анимация
6. **Экспорт/импорт** — JSON-файл, импорт с merge по ID (без дублей)
7. **Плавающая панель** — `position: fixed`, `translate(-50%,-50%)` для центрирования, drag за заголовок (сохранение позиции в localStorage), resize handle снизу (сохранение высоты)
8. **Закрытие по клику вне** — `mousedown` на document
9. **Блокировка скролла** — `wheel` listener предотвращает скролл колонки за пределами списка
10. **Удаление** — двойной клик (pending 2.5s, как в блоках) вместо `confirm()`
11. **Intelligence** — сохранение шаблонов с `createdAt`/`useCount`

**Коммиты:**
- `c97fc93` — 6 фич (лимит, статистика, дублирование, rename, превью, экспорт/импорт)
- `29a94f2` — плавающая панель + resize
- `e87dbc4` — resize handle внутрь, превью hover на snap-info
- `44388bc` — превью только по кнопке, drag за заголовок
- `021a7a7` — больше пространства срабатывания
- `993b7d9` — preview position:absolute
- `7d62652` — preview вынесен из snap-info
- `98dbc48` — pointer-events:none, только кнопка
- `3210fbd` — клик вне, фон preview, блокировка скролла
- `68795aa` — центрирование, удаление по двойному клику

### TextFormat — кнопка (сессия)

**Файлы:** `text-format.js`

**Изменения:**
- `updateButtonIcon()` — обновляет ВСЕ кнопки `.tf-btn` при вызове без аргумента (а не только текущую)
- `createButton()` — передаёт `btn` явно в `updateButtonIcon(btn)` (кнопка ещё не в DOM)

**Коммиты:** `77517b2`, `0956064`

### Тосты — единый стиль (сессия)

**Файлы:** `styles.css`, `qr-panel.js`

**Изменения:**
- `.toast` — фон `rgba(12,32,42,0.95)` (teal), `backdrop-filter: blur(12px)`, pill-радиус
- `.toast.success` — `#4ade80` (яркий зелёный) + мягкая зелёная рамка
- `.toast.error` — `rgba(40,18,22,0.95)` (тёмно-красный) + `#f87171`
- `.toast.info` — нейтральный `var(--text1)`
- `.qr-toast` — приведён к тому же стилю (pill, transition вместо `dropIn`)
- `qr-panel.js` — переключение через класс `.show` вместо `display`

**Коммит:** `cb1549c`

### Кнопка Поиск — toggle (сессия)

**Файлы:** `ui.js`, `blocks.js`, `app.js`

**Изменения:**
- `Search.isOpen()` экспортирован из UI
- Кнопка блока и глобальная `btn-search` — toggle: если открыта → `close()`, иначе → `open()`

**Коммит:** `0baf26e`

### Undo/Redo — исправления (сессия)

**Файлы:** `llm-features.js`, `ai-transform.js`, `state.js`, `blocks.js`

**Изменения:**
- **Тезаурус** — `setRangeText` → `execCommand('insertText')` (нативный Ctrl+Z работает)
- **AI-трансформация** — `setRangeText`/`value =` → `execCommand('insertText')`
- **Block undo** — `blockSnapshot(blockId, selStart, selEnd)` сохраняет позицию курсора, `blockUndo/blockRedo` возвращают `{ start, end }` для восстановления
...
- **Timer `_applyArc` CCW direction**: исправлено — возврат к старой логике из `timer_old.js`. Направление визуала определяется ПУТЁМ (CW/CCW sweep-flags в `_buildPath`), НЕ инверсией progress или dashoffset. `progress` всегда 0→1 для обоих направлений. CCW progress: `(elapsed % 60) / 60` (per-minute), не `overallProgress` (1→0). `_applyArc` идентичен для CW/CCW: `dashoffset=''`, `-(headPos-hLen)`, `visualProgress*N`. DOM-based `_cachePoints` (getPointAtLength).
- **Timer `_applyArc` оптимизация**: static writes (display, strokeDashoffset='0', r='2') — один раз за arc-сессию (`_arcStaticsApplied`). `arcHeadSeg.strokeDasharray` кэш (`_lastHeadSegDasharray`) — меняется только первые ~6с минуты. Wrap detection: `progress < _lastProgress` → `_resetArcFrameCache()`. `_resetArcFrameCache()` вызывается из `_invalidateCaches`, `_resetRenderState`, `resumeTimer`, `_hideArc`, `destroy`. DOM-записи: 10→4/кадр (после 6с), 300→120/s.
- **Timer `_updateDisplay` оптимизация**: `_oldDigitEl` ref вместо `querySelectorAll('.timer-digit-old')`. `_prevMin !== minutes` вместо `+valueEl.textContent` DOM-read. Убраны DOM-scan + NodeList аллокация + числовое приведение.
- **Timer misc оптимизации**: `saveState` — JSON.stringify кэш (`_lastSavedState`). `onPointerDown` — inline style вместо `getComputedStyle`. `onPointerMove` — distance² вместо `Math.hypot`. `startPulse` — `_pulseUrgentApplied` flag (30 ops/classList). `_checkCornerGlow` — `_endCornerGlow` callback. `_playCompletionSound` — `COMPLETION_FREQS` константа + for. `openInlineInput` — extracted handlers. `_invalidateCaches` — `_initialized` guard. `destroy` — null DOM refs.
...
### Preview — QR-кнопка +间距

**Файлы:** `index.html`, `styles.css`, `app.js`

**QR-кнопка** (`#prev-qr`) в шапке превью передdivider:
- SVG-иконка QR-кода
- Proxy-объект с getter `value → Preview.getText()`
- **Баг:** при переключении QR между текстовым блоком и превью бейджи показывают данные предыдущего источника. Откат两个 попытки фикса (`5591ab8`, `03700f2`) — требует deeper investigation.
...
**QR-кнопка** (`#prev-qr`) в шапке превью передdivider:
- SVG-иконка QR-кода
- Proxy-объект с getter `value → Preview.getText()` — QR-панель всегда получает актуальный текст
- Fallback `requestAnimationFrame`: если `.qr-info-row` не найден после первого рендера — создаёт и перерисовывает (фикс отсутствия бейджей "Сжатие", "К.о.", "байт" при первом открытии из превью)
...
### Anchors — marker position fix + sticky indicators

**Файл:** `anchors.js`, `styles.css`

**Исправленные баги:**
1. **`_measurePos` — mirror использовал полный текст** — `mirror.textContent = ta.value` вместо `substring(0, charPos)`. Смещало границу текстового узла → маркер выше реальной позиции. Восстановлена обрезка из `anchors_old.js`.
2. **Mirror `word-break: break-all`** — textarea использует дефолт `normal`. Разные точки переноса → дрифт ~1 строка на 1000 символов. Убран из mirror.
3. **`taCs` не объявлена в `_renderMarkers`** — ReferenceError при `bgHighlight`. Восстановлено `const taCs = getComputedStyle(ta)`.
4. **`DEFAULT_MARKER_COLOR`** → `#1DD110`.

**Новая фича — sticky indicators:**
- Боковой маркер **всегда виден**, даже когда якорь за пределами видимой области
- Якорь выше → маркер залипает сверху; ниже → снизу
...
- **Alt+Shift+F:** открывает меню из клавиатуры (Shift+F конфликтует с вводом "А" на русской раскладке)
...
- `app.js:747-754` — Alt+Shift+F shortcut (Shift+F → Alt+Shift+F, fix: русская раскладка)
