...
21. ⏸️ **Откат** — пробовали 9 коммитов, не зашло. Подходы: CSS scroll-padding-bottom (не работает на textarea), JS scrollTop (только с пустыми строками после курсора), виртуальные \n через value getter/setter (дёргается, подсветка строки на 1 строчке). Проблема: textarea不同于div — scroll-padding-bottom не поддерживается, а JS-прокрутка требует контент ниже курсора. Оставить на потом

### MindMap — визуализация мышления

22. ✅ **mindmap.js + mindmap.css** — новый модуль. Кнопка 🧠 в шапке превью. LLM анализирует текст → JSON (слова, связи, кластеры, дерево аргументов). 4 режима: W/G/T/C. Оверлей с backdrop-filter, кнопки появляются при proximity 150px. Glow/shadow SVG-фильтры. Escape/ПКМ закрывают
...
```
51bc007 refactor: MindMap CSS вынесен в mindmap.css
20f123f feat: MindMap — proximity кнопки 150px, glow/shadow фильтры
b2a977a feat: MindMap — визуализация мышления превью (4 режима)
92eaa28 fix: восстановлен CSS completion checkbox после отката тайпрайтера
revert: откат тайпрайтера (9 коммитов) — не зашло
...
- `mindmap.js` (~400 строк) — MindMap: SVG-визуализация (слова/граф/дерево/кластеры)
- `mindmap.css` (~60 строк) — стили оверлея, кнопок, панели
