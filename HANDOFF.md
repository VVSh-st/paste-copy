...
- **Timer pause/resume**: ЛКМ на иконке: idle→start, active→pause, paused→resume, pulse→reset. `pauseTimer()`: `stopTick()`, `_hideArc()`, dim + `timer-paused` class. `resumeTimer()`: `startTs = Date.now() - _pausedElapsed`, `_runStartPerf` пересчитан, `_lastDir = null` (форсирует re-show arc). Пауза сохраняется в localStorage (`pausedElapsed`). При F5 восстанавливается в paused состоянии (dim, без tick).
