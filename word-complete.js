...
const InlineHint = (() => {
  ...
  document.addEventListener('selectionchange', () => {
    ...
    const ac = activeTa.value.charAt(pos);
    const bc = activeTa.value.charAt(pos - 1);
    if ((ac && /[\p{L}\p{N}_]/u.test(ac)) ||
        (bc === ' ' && ac && /\p{L}/u.test(ac))) hide();
  });
...
const WordComplete = (() => {
  ...
  function handleInput(ta) {
    ...
    if ((afterChar && /[\p{L}\p{N}_]/u.test(afterChar)) ||
        (beforeChar === ' ' && afterChar && /\p{L}/u.test(afterChar))) {
      InlineHint.hide(); return;
    }
