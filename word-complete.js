...
const InlineHint = (() => {
  ...
  function _renderList() {
    ...
    listEl.innerHTML = _candidates.map((w, i) => {
      ...
      const suffixOpacity = i === _selectedIndex ? '1' : '0.45';
      return `<div style="padding:1px 0"><span style="opacity:0.3">${_escHtml(prefix)}</span><span style="opacity:${suffixOpacity}">${_escHtml(suffixPart)}</span></div>`;
    }).join('');
  }
