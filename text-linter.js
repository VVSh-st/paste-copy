...
window.TextLinter = (() => {
  ...
  const HINT_RULES = [
    { id: 'comma-chto', re: /(?:^|[^\p{L}\p{N}_])(?:то|так|думаю|кажется|важно|нужно|понятно|видно)\s+что(?:$|[^\p{L}\p{N}_])/giu, text: 'возможна запятая перед «что»' },
    { id: 'comma-esli', re: /(?:^|[^\p{L}\p{N}_])если(?:$|[^\p{L}\p{N}_])/giu, text: 'проверь запятую перед/после «если»' },
  ...
  function processNormalSegment(segment, opts, stats, hints, lineOffset = 0) {
    ...
    if (opts.collapseBlankLines) {
      text = text.replace(/\n{3,}/g, match => {
        inc(stats, 'blanks', Math.max(1, match.length - 2));
        return '\n\n';
      });
    }
